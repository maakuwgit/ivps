...
Public Class GetThumb : Implements IHttpHandler
    ...
    Public Sub ProcessRequest(ByVal ctx As HttpContext) Implements IHttpHandler.ProcessRequest
        ...
        If Not ctx.Request("Id") Is Nothing Then
            ...
            If IsNumeric(ctx.Request("Id")) Then
                ...
                Dim ad As New SqlDataAdapter("Select Data, DataType from RCProduct where Id=@Id", con)
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    ctx.Response.ContentType = dt.Rows(0).Item("DataType")
                    If Not ctx.Request("w") Is Nothing Then
                        ...
                        Dim ms2 As IO.MemoryStream =  ImageUtility.ResizeImage(ms, w, 0)
                        ' ResizeImage(m As IO.MemoryStream, Optional ByVal w As Integer = 0, Optional ByVal h As Integer = 0) As IO.MemoryStream
                        ctx.Response.BinaryWrite(ms2.ToArray)
                    ElseIf Not ctx.Request("h") Is Nothing Then
                        Dim h As Integer = CInt(ctx.Request("h"))
                        Dim ms As New IO.MemoryStream(dt.Rows(0).Item("Data"), False)
                        Dim ms2 As IO.MemoryStream = ImageUtility.ResizeImage(ms, 0, h)
                        ctx.Response.BinaryWrite(ms2.ToArray)
    ...
    Protected Function ResizeImage2(m As IO.MemoryStream, ImgType As String, Optional ByVal w As Integer = 0, Optional ByVal h As Integer = 0) As IO.MemoryStream
