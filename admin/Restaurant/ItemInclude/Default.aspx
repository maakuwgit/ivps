...
<asp:Content ID="Content2" ContentPlaceHolderID="main" Runat="Server">
	...
	<div class="container-fluid">
		...
		<div class="row">
		<a class="btn btn-secondary mb-3" title="Edit" href="<%= baseurl %>/iteminclude/edit.aspx"><i class="fas fa-plus-circle"></i> Includes</a>
		</div>
		...
		<table class="table table-striped table-bordered table-hover border-top-0" id="searchableTable">
			...
			%>
				<tr>
					<td><%= row("id") %></td>
					<td class="col"><a href="<%= baseurl %>/iteminclude/edit.aspx?id=<%= row("id") %>"><%= WordLimit(row("Items"), 50) %></a></td>
					<td class="text-right">
							...
							<a class="btn btn-outline-success" title="Edit" href="<%= baseurl %>/iteminclude/edit.aspx?id=<%= row("id") %>"><i class="fas fa-edit"></i></a>
						</div>
					</td>
				</tr>
			<%
