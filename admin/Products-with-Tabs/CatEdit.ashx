...
Public Class Edit : Implements IHttpHandler
    ...
    Public Sub ProcessRequest(ByVal ctx As HttpContext) Implements IHttpHandler.ProcessRequest
        ...
        If Not ctx.Request("Name") Is Nothing Then
            ...
            If id = 0 Then
                cmd = New SqlCommand("Insert Into RCProductCat2 (Name, Description, ParentId) Values (@Name, @Description, @ParentId)", con)
            Else
                cmd = New SqlCommand("Update RCProductCat2 set Name=@Name, Description=@Description where Id=@Id", con)
