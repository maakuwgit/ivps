...
Public Module Forms
	...
	Public Function SessionGuid
		if _SessionGuid.length = 0 Then
			_SessionGuid = System.Guid.NewGuid.ToString()
			HttpContext.Current.Session(SESSION_GUID) = _SessionGuid
		End If
		return _SessionGuid
	End Function
	'
	' Evertime  the session variable is validated it's then destroyed to prevent it being used again
	''
	Public Function ValidSessionGuid(guid)
		Dim result = String.Compare(guid, _SessionGuid) = 0
		_SessionGuid = ""
		return result
	End Function
...
Public Class Email
	...
	Public Function IncludeInEmail(Item As String)
		...
		Item.ToLower().StartsWith("g-recaptcha") And Not _ 
		Item.ToLower().StartsWith(Forms.SESSION_GUID)  Then
			return True
		End If
			return false
	End Function
